...
int completed[MAX_LINES];
MPI_Request recvReq[MAX_LINES];
int core[MAX_LINES];
int frontier[MAX_THREADS];
...
void main (int argc, char **argv)
{
	...
	{	    
		...
		for (j=i*blockSize; j<(i+1)*blockSize; ++j)
		    core[tasks[j]] = i+1;
		frontier[i] = i*blockSize;
	    }
	    
	    //Receive points to draw
	    int iter;
	    temp = malloc( sizeof(int) * Y_RESN );
	    int taskLine;
	    for (taskLine=0; taskLine<X_RESN; ++taskLine)
	    {
		// Receive number of points to draw
		MPI_Irecv(&numbers[taskLine], 1, MPI_INT, core[taskLine], taskLine, MPI_COMM_WORLD, &recvReq[taskLine]);
		//printf("irecv %d %d\n", core[taskLine], taskLine);
	    }
	    
	    for (iter=0; iter<X_RESN; ++iter)
	    {
		for (i=0; i<nSubProcesses; ++i)
		    if (frontier[i]<X2 && !completed[tasks[frontier[i]]])
		    {
			int success;
			if (tasks[frontier[i]]>=X_RESN) {
			    ++frontier[i];
			    continue;
			}
			MPI_Test(&recvReq[tasks[frontier[i]]], &success, MPI_STATUS_IGNORE);
			if (success)
			    break;
		    }
		if (i==nSubProcesses)
		...
		int task = tasks[frontier[i]];
		frontier[i]++;
		i = task;
		completed[i] = 1;
		// Receive points
		MPI_Recv(temp, numbers[i], MPI_INT, core[i], i+X2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);		
#ifdef X_ENABLED
		//XWindow Draw		    
		for (j=0; j<numbers[i]; ++j)
		{
		    XDrawPoint(display, win, gc, i, temp[j]);
		    if (i==50)
			printf("p %d\n", temp[j]);
		}
#endif 
	...
	{	    
	    ...
	    //Do calculations	    
	    int *temp = malloc( sizeof(int) * Y_RESN );		//Storage for points to draw
	    int x;
